...
class Account{
    ...
    function viewAccounts($school_year, $semester) {
        try {
            ...
            error_log("Error in viewAccounts: " . $e->getMessage());
            return [];
        }
    }

    function searchAccounts($searchQuery, $school_year, $semester) {
        try {
            ...
            $sql = "SELECT * FROM account 
                    ...
                    AND school_year = :school_year 
                    AND semester = :semester 
                    AND (StudentID LIKE :search 
                        OR first_name LIKE :search 
                        OR last_name LIKE :search 
                        OR WmsuEmail LIKE :search 
                        OR Course LIKE :search)
                    ORDER BY last_name ASC";
            
            $qry = $this->db->connect()->prepare($sql);
            $qry->bindParam(':search', $searchTerm, PDO::PARAM_STR);
            $qry->bindParam(':school_year', $school_year, PDO::PARAM_STR);
            $qry->bindParam(':semester', $semester, PDO::PARAM_STR);
 ...
 */
function createStudAcc($studentId, $first_name, $last_name, $mi, $wmsuEmail, $password, $role, $course, $year, $section, $school_year, $semester) {
    try {
        // Create account
        $sql = "INSERT INTO account (
            studentID, first_name, last_name, mi, wmsuEmail, 
            password, role, course, year, section, 
